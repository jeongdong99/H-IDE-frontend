...
import { checkUserid, checkNickname, signUpUser, sendEmailCode } from '../../utils/api'; //api.js 에 작성하신 함수를 여기 import 해주셔야 합니다!
        ...
        <form className='flex flex-col items-center' onSubmit={handleSubmit(onSubmit)}>
          <div className='mb-2 w-full max-w-md'>
            ...
            <div className='flex items-center mt-2'>
              <input
                ...
                {...register('id', userId)}
              />
              <button onClick={handleCheckUserid} className='px-4 py-1.5 ml-4 hover:bg-[#457D61] duration-200 bg-[#457D61] text-sm font-semibold text-white rounded-md'>중복 확인</button>
            </div>
            {
              useridMessage &&
              <div>
                <span className={useridMessage.includes("사용 가능한") ? 'text-green-500' : 'text-red-500'}>
                  {useridMessage}
                </span>
              </div>
            }
          ...
          <div className='mb-2 w-full max-w-md'>
            ...
            <div className='flex items-center mt-2'>
              ...
              />
              <button onClick={handleCheckNickname} className='px-4 py-1.5 ml-4 hover:bg-[#457D61] duration-200 bg-[#457D61] text-sm font-semibold text-white rounded-md'>중복 확인</button>
            </div>
            {
              nicknameMessage &&
              <div>
                <span className={nicknameMessage.includes("사용 가능한") ? 'text-green-500' : 'text-red-500'}>
                  {nicknameMessage}
                </span>
              </div>
            }
          ...
          <div className="mb-2 w-full max-w-md">
            ...
            {errors?.checkPassword && (
              <div>
                <span className="text-red-500">{errors.checkPassword.message}</span>
              </div>
            )}
          </div>
